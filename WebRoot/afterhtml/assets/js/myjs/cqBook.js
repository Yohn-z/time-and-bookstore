...
function pagereload2(id){
	

	   $.ajax({
	           url:'/bookshophhit/admin/borrow/bbztlist.action?jyztId=5&pageNo=1&pageSize=7',
	           type:'POST',
	           dataType:'JSON',
	           success:function (data) {
	        	   ...
	        	   for(var i=0;i<data.data.list.length;i++){
	        		   ...
	        		   if(data.data.list[i].jyztId == 5){
	        			   $("#dataTables-example2 tbody").append(
											   	          				'<tr><td>'
													+ data.data.list[i].tsjyId
													+ '</td><td>'
													+ data.data.list[i].tsId
													+ '</td><td>'
													+ data.data.list[i].uId
													+ '</td><td>'
													+ data.data.list[i].tsjyFtime
													+ '</td><td>'
													+ data.data.list[i].tsjyNum
													+ '</td><td style="color:red">'
													+ jyzt
													+ '</td><td><button type="button" class="btn btn-danger btn-sm jj" data-toggle="modal" data-target="#myjjModal"  data-uid='
													+ data.data.list[i].uId
													+ ' data-id="'
													+ data.data.list[i].tsjyId
													+ '">拒绝</button></td></tr>');
	     	 ...
	     	 $(id).bootstrapPaginator({
	 		    ...
	 		    itemTexts: function (type, page, current) {
	 		        ...
	 		        onPageClicked: function (event, originalEvent, type, page){//给每个页眉绑定一个事件，其实就是ajax请求，其中page变量为当前点击的页上的数字。
	 		            cupa2 = page;
	 		        	$.ajax({
	 		                url:'/bookshophhit/admin/borrow/bbztlist.action?jyztId=5&pageNo='+page+'&pageSize=7',
	 		                type:'POST',
...
function pagetworeload2(id,currentPage){
	
	 var getTimestamp=new Date().getTime();  
       
	  
	   $.ajax({
	           url:'/bookshophhit/admin/borrow/bbztlist.action?jyztId=5&pageNo='+currentPage+'&pageSize=7&time='+getTimestamp,
	           type:'POST',
	           dataType:'JSON',
	           success:function(data) {
	        	   cupa2 = 1;
			  ...
			  if(data.code=="1"){
				...
				now.parent().parent().fadeTo("slow", 0.01, function(){//fade
					now.parent().parent().slideUp("slow", function() {//slide up
				    	...
				    	pagetworeload2("#pageLimit2",cupa2);
				    });
				  });
				  ...
				  $.ajax({
					  ...
					  success:function(data){
						  ...
						  if(data.code=="1"){
							  ...
							  pagetworeload2('#pageLimit2',cupa2);
							  //pagetworeload1('#pageLimit',cupa1);
						  }
						  else {
							  demo.showNotification('bottom','center',3,data.msg);
						  }
